...
#include "simple_maze_solver_engine.h"
#include "pc_uart.h"
...
SimpleMazeSolverEngine::SimpleMazeSolverEngine(int red_level_in, bool debug)
: level(red_level_in), updated_local_x(0), updated_local_z(66), 
ms_died(false), heading(0), just_turned_left_flag(false), debug_mode(debug)
{
...
void SimpleMazeSolverEngine::update_location()
{
	if(debug_mode) { 
...
void SimpleMazeSolverEngine::move_forward(int distance_in_um)
{
...
void SimpleMazeSolverEngine::rotate_raw(int raw_angle)
{
...
void SimpleMazeSolverEngine::rotate_deg(int angle_deg)
{
...
int SimpleMazeSolverEngine::decide_next_move()
{
	int return_value = -2;
...
void SimpleMazeSolverEngine::do_next_move(int move)
{	
...
void SimpleMazeSolverEngine::print_status()
{
...
void SimpleMazeSolverEngine::run()
{
...
//	while( ! key_ready() && !ms_died && !b1->pressed() && !b2->pressed() && !b3->pressed())
	while(!ms_died && !b1->pressed() && !b2->pressed() && !b3->pressed())
	{
		evaluate_scene(updated_local_x, updated_local_z);
	...
	if(ms_died)
	{
		...
		write_line("Maze Solver FAILED");
	}
}

void SimpleMazeSolverEngine::evaluate_scene(int estimated_x, int estimated_z)
{
	...
	{
		write_line("maze solver failed - ipc failed");
		ms_died = true;
	}
	else
	{
