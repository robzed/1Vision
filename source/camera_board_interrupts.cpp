...
void __attribute__ ((interrupt("IRQ"))) vsync_interrupt (void)
{
	...
	GPIO_IOCLR = _CC3_BUF_WEE;        //BUF_WEE=0
...
void enable_vsync_interrupt (void)
{
	...
	// enable interrupts for vsync
	VICVectAddr2 = (unsigned long)vsync_interrupt;   // set interrupt vector in 0
	VICVectCntl2 = VICVectCntl_ENABLE | VIC_Channel_EINT0; // use it for Timer 1 Interrupt:
	VICIntEnable = (1<<VIC_Channel_EINT0);    // Enable Timer 1 Interrupt		
