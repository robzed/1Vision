...
// Converted from BASIC & Dephi by Rob Probin
//
...
//
// Basic original: http://www.totse.com/en/technology/telecommunications/wxmodem.html
//
// Verified here http://www.lammertbies.nl/comm/info/crc-calculation.html?crc=123456789&method=ascii
// Delphi code http://www.efg2.com/Lab/Library/UseNet/1999/0630b.txt
//
...
unsigned short xmodem_crc_table[256] = {
...
unsigned short xmodem_crc(unsigned char const *message, int number_bytes)
{
	...
	{		
		crc = (crc << 8) ^ xmodem_crc_table[crc>>8 ^ *message];
		message++;
	}
...
{
	unsigned char  test1[] = "123456789";
	printf("The xmodem_crc() of \"%s\" is 0x%X\n", test1, xmodem_crc(test1, strlen(test1)));

	unsigned char  test2[] = "987654321";
	printf("The xmodem_crc() of \"%s\" is 0x%X\n", test2, xmodem_crc(test2, strlen(test2)));

	unsigned char  test3[] ="11122233344445";
	printf("The xmodem_crc() of \"%s\" is 0x%X\n", test3, xmodem_crc(test3, strlen(test3)));
